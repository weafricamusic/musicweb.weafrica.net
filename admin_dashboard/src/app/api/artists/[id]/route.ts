...
export async function GET(_req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
	...
	const attempts = [
		'id,stage_name,avatar_url,bio,country,verified,approved,status,blocked,created_at',
		'id,stage_name,verified,approved,status,blocked,created_at',
		'id,stage_name,approved,created_at',
		'id,stage_name,created_at',
	] as const

	let lastError: unknown = null
	for (const select of attempts) {
		let query = supabase.from('artists').select(select).eq('id', id)
		...
		if (!error) {
			...
			return NextResponse.json({ ok: true, artist: data }, { headers: { 'cache-control': 'no-store' } })
		}
		lastError = error
	}
