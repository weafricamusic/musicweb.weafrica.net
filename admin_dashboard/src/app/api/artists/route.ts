...
export async function GET(req: NextRequest) {
	...
	const verifiedOnly = (url.searchParams.get('verified') ?? 'true').toLowerCase() !== 'false'

	const rangeTo = offset + limit - 1

	const attempts: Array<{
		select: string
		filters: Array<'approved' | 'status' | 'blocked' | 'verified'>
	}> = [
		{
			select: 'id,stage_name,avatar_url,bio,country,verified,approved,status,blocked,created_at',
			filters: ['approved', 'status', 'blocked', 'verified'],
		},
		{
			select: 'id,stage_name,verified,approved,status,blocked,created_at',
			filters: ['approved', 'status', 'blocked', 'verified'],
		},
		{
	...
	for (const a of attempts) {
		...
		if (verifiedOnly && a.filters.includes('verified')) query = query.eq('verified', true)

		const { data, error } = await query
		if (!error) {
			return NextResponse.json(
				{ ok: true, artists: data ?? [], limit, offset },
