...
type Filter = 'pending' | 'live' | 'removed' | 'all'

function normalizeFilter(value: unknown): Filter {
	const v = String(value ?? '').trim().toLowerCase()
	if (v === 'pending' || v === 'live' || v === 'removed' || v === 'all') return v
	return 'pending'
}
...
export async function GET(req: Request) {
	...
	if (filter === 'removed') q = q.eq('is_active', false)

	const { data, error } = await q
	if (error) {
		if (isSchemaMissingError(error.message)) {
			return json(
				{ error: "songs table not found. Apply migrations and then run: NOTIFY pgrst, 'reload schema';" },
				{ status: 500 },
			)
		}
	...
	return json({ ok: true, tracks: data ?? [], filter })
}
...
export async function PATCH(req: Request) {
	...
	const { data: beforeState, error: beforeError } = await supabase.from('songs').select('*').eq('id', id).single()
	if (beforeError) {
	...
	return json({ ok: true, track: data })
}
