...
import { isArtistPlan } from '@/lib/subscription/admin-plan-scope'
...
export default async function AdminSubscriptionsArtistsPage() {
	...
	const artistPlans = ((plans ?? []) as PlanRow[]).filter((p) => isArtistPlan(p))
	...
	return (
		<div className="space-y-6">
			<div className="flex items-center justify-between">
				<div>
					<h1 className="text-2xl font-bold">Artist Subscriptions</h1>
					<p className="mt-1 text-sm text-gray-400">Manage artist subscription plans and assignments.</p>
				</div>
				<div className="flex flex-wrap gap-2">
					...
					<Link href="/admin/subscriptions/user-subscriptions?audience=artist" className="inline-flex h-10 items-center rounded-xl border border-white/10 px-4 text-sm hover:bg-white/5">User subs</Link>
				</div>
			</div>
					...
					<table className="w-full min-w-[980px] border-separate border-spacing-0 text-left text-sm">
						...
						<tbody>
							{artistPlans.length ? (
								...
								<tr>
									<td colSpan={5} className="py-6 text-sm text-gray-400">No artist plans found.</td>
								</tr>
							)}
						</tbody>
					</table>
			...
			<SubscriptionsToolsClient audience="artist" />
		</div>
	)
}
