...
import { getArtistVerificationRows } from '../../actions'
import { ArtistVerificationTable } from '../../ArtistVerificationTable'
...
export default async function ArtistVerificationBucketPage(props: { params: Promise<{ bucket: string }> }) {
	...
	let rows: Awaited<ReturnType<typeof getArtistVerificationRows>> = []
	let loadError: string | null = null
	try {
		rows = await getArtistVerificationRows(bucket)
	} catch (e) {
		loadError = e instanceof Error ? e.message : 'Failed to load artists'
	}
	...
	return (
		<div className="space-y-4">
			<div className="flex flex-wrap gap-2">
				...
				<Link href="/admin/verification/artists/approved" className="inline-flex h-10 items-center rounded-xl border border-white/10 px-4 text-sm hover:bg-white/5">
					Approved
				</Link>
				<Link href="/admin/verification/artists/rejected" className="inline-flex h-10 items-center rounded-xl border border-white/10 px-4 text-sm hover:bg-white/5">
					Rejected
				</Link>
			</div>
			...
			{loadError ? (
				<div className="rounded-2xl border border-red-500/30 bg-red-500/10 p-4 text-sm text-red-200">
					<div className="font-medium">Artist list unavailable</div>
					<div className="mt-1 opacity-90">{loadError}</div>
				</div>
			) : (
				<div className="rounded-2xl border border-white/10 bg-white/5 p-6">
					<ArtistVerificationTable rows={rows} />
				</div>
			)}
		</div>
	)
