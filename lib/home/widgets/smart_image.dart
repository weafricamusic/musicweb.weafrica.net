...
/// FIXED SmartImage that handles ALL Supabase URL formats
  ...
  List<String> _getPossibleUrls(String input) {
    ...
    if (path.startsWith('http')) {
      ...
      final normalized = parsed == null
          ? path
          : (SupabaseMediaUrl.normalize(parsed)?.toString() ?? path);
      ...
      if (baseUrl.isEmpty) {
        debugPrint('   ⚠️ SupabaseEnv.supabaseUrl is empty; cannot build image URL candidates.');
        return [];
      }
      ...
      // Normalize legacy bucket alias if a stored path includes it.
      if (path.startsWith('song_thumbnails/')) {
        path = path.replaceFirst('song_thumbnails/', 'song-thumbnails/');
      }

      // Encode path segments (keeps slashes intact) so spaces/special characters work.
      ...
      urls.add('$baseUrl/storage/v1/object/public/song-thumbnails/$encodedPath');
      urls.add('$baseUrl/storage/v1/object/song-thumbnails/$encodedPath');
      
      // Format 5: Try in different buckets
      urls.add('$baseUrl/storage/v1/object/public/thumbnails/$encodedPath');
      urls.add('$baseUrl/storage/v1/object/public/video_thumbnails/$encodedPath');
      urls.add('$baseUrl/storage/v1/object/public/media/$encodedPath');
      urls.add('$baseUrl/storage/v1/object/public/videos/$encodedPath');
    }
