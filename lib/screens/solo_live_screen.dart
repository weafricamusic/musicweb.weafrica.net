...
import '../features/live_old/services/live_session_service.dart' as legacy_live;
...
class _SoloLiveScreenState extends State<SoloLiveScreen>
  ...
  // Live animation popups
  String? _giftPopText;
  String? _earningsPopText;
  String? _goalCompleteText;
  ...
  Future<void> _initializeAgoraHost() async {
    try {
      final channelId = widget.channelId ?? 'solo_${widget.liveSessionId ?? DateTime.now().millisecondsSinceEpoch}';

      _agoraEngine = createAgoraRtcEngine();
      await _agoraEngine!.initialize(
        RtcEngineContext(appId: AppEnv.agoraAppId.trim()),
      );
      ...
      debugPrint('FAILED TO START AGORA HOST: $e');
  ...
  void _sendGift(String emoji, String name, int coins) {
    setState(() {
      _totalCoins += coins;
      _totalEarnings += coins;
      _giftPopText = '$emoji $name';
      _earningsPopText = '+$coins coins';
      _updateGoalProgress('flowers', coins);

      final flowerGoal = _goals.firstWhere((g) => g.id == 'flowers');
      if (flowerGoal.current >= flowerGoal.target) {
        _goalCompleteText = '🎉 ${flowerGoal.title} Goal Complete!';
      }
    });
  ...
  Widget _summaryRow(String label, String value) {
    ...
    Future<void> _endLive() async {
      ...
      try {
        debugPrint('🔴 END LIVE clicked liveSessionId=${widget.liveSessionId} channelId=${widget.channelId}');

        if (widget.liveSessionId != null && widget.liveSessionId!.isNotEmpty) {
          // Prefer server-backed service which performs service-role writes.
          final ch = (widget.channelId ?? '').trim();
          if (ch.isNotEmpty) {
            try {
              final currentUserId = Supabase.instance.client.auth.currentUser?.id ?? '';
              if (currentUserId.isNotEmpty) {
                await legacy_live.LiveSessionService()
                    .endLiveAndEnsureCleared(hostId: currentUserId, channelId: ch);
                debugPrint('✅ legacy endLiveAndEnsureCleared completed for channel=$ch');
              } else {
                await legacy_live.LiveSessionService().endLive(channelId: ch);
                debugPrint('✅ legacy LiveSessionService.endLive completed for channel=$ch');
              }
            } catch (e, st) {
              debugPrint('❌ legacy end failed: $e');
              debugPrint('$st');
              // Try client-side service as a secondary fallback
              try {
                await LiveSessionService().endLiveSession(widget.liveSessionId!);
                debugPrint('✅ LiveSessionService.endLiveSession completed for ${widget.liveSessionId}');
              } catch (e2, st2) {
                debugPrint('⚠️ client endLiveSession failed: $e2');
                debugPrint('$st2');
                // Last-resort: attempt direct Supabase update and log any RLS error
                try {
                  ...
                  debugPrint('✅ Direct fallback update response: $updated');
                } catch (e3, st3) {
                  debugPrint('❌ Direct Supabase update failed (likely RLS): $e3');
                  debugPrint('$st3');
                }
              }
            }
          } else {
            // No channelId available - try ending by session id via client service
            try {
              await LiveSessionService().endLiveSession(widget.liveSessionId!);
              debugPrint('✅ LiveSessionService.endLiveSession completed for ${widget.liveSessionId}');
            } catch (e, st) {
              debugPrint('❌ end by sessionId failed: $e');
              debugPrint('$st');
            }
          }

          // Ensure Agora disconnects immediately
          try {
            await _agoraEngine?.leaveChannel();
            await _agoraEngine?.release();
            debugPrint('✅ Agora engine left and released');
          } catch (e) {
            debugPrint('❌ Failed to leave/release Agora engine: $e');
          }

          // Dispose camera and cancel subscriptions to stop local activity
          try {
            await _cameraController?.dispose();
          } catch (_) {}
          ...
          debugPrint('✅ Live ended in backend and local cleanup done: ${widget.liveSessionId}');
        } else {
          debugPrint('❌ liveSessionId missing. Backend live not ended.');
        }
      } catch (e) {
        debugPrint('❌ Failed to end live: $e');
      }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          // Camera Preview
          _isAgoraReady && _agoraEngine != null && !_isCameraOff
              ? SizedBox.expand(
          ...
          // Earnings pop animation
          if (_earningsPopText != null)
            Positioned(
              top: 155,
              right: 20,
              child: Text(
