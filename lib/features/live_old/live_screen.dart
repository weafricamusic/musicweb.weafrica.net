...
class _LiveScreenState extends State<LiveScreen> {
  ...
  bool _shouldJoinAsBroadcaster(String userId) {
    final uid = userId.trim();
    if (uid.isEmpty) return false;

    if (widget.args.isBattle == true) {
      if (_battle != null) {
        final c1 = _battle!.competitor1Id.trim();
        final c2 = _battle!.competitor2Id.trim();

        if (uid == c1 || uid == c2) {
          debugPrint(
            '🔴 USER IS PERFORMER from battle data: uid=$uid c1=$c1 c2=$c2',
          );
          return true;
        }
      }

      final battleArtists = widget.args.battleArtists ?? const <String>[];
      final isInArgs = battleArtists.map((e) => e.trim()).contains(uid);

      if (isInArgs) {
        debugPrint('🔴 USER IS PERFORMER from args: uid=$uid');
        return true;
      }
    }

    final hostId = widget.args.hostId?.trim() ?? '';
    if (hostId.isNotEmpty && uid == hostId) {
      debugPrint('🔴 USER IS HOST: uid=$uid hostId=$hostId');
      return true;
    }

    debugPrint('🟢 USER IS AUDIENCE: uid=$uid');
    return false;
  }
  ...
  Future<void> _loadSolo() async {
    try {
      ...
      final shouldBroadcast = _shouldJoinAsBroadcaster(
        currentUserId.isNotEmpty ? currentUserId : joinUserId,
      );

      debugPrint(
        'LiveSession: userId=$joinUserId, role=${widget.args.role?.id ?? 0}, '
        'isBroadcaster=$shouldBroadcast, sessionType=solo',
      );
      ...
      if (session == null &&
          LivePriorityAccessGate.instance.wasBlockedRecently(
            widget.args.channelId ?? '',
          )) {
        ...
        if (upgraded) {
          ...
          joinFailureMessage = switch (retry) {
            AppFailure<live_model.LiveSession>(:final userMessage) =>
              userMessage,
  ...
  Future<void> _loadBattle() async {
    try {
      ...
      debugPrint(
        'LiveSession: userId=$joinUserId, role=${widget.args.role?.id ?? 0}, '
        'isBroadcaster=$shouldBroadcast, sessionType=battle',
      );

      debugPrint(
        'LiveBattleArgs: channelId=${widget.args.channelId}, '
        'battleId=${widget.args.battleId}, liveId=${widget.args.liveId}',
      );
  ...
  Widget _withQaDebugBanner({
    ...
    final branch =
        '${isBroadcaster ? 'Host' : 'Fan'} ${sessionType == 'battle' ? 'Battle' : 'Solo'}';
        ...
        Positioned(
          ...
          child: SafeArea(
            child: GestureDetector(
              onTap: () {
                setState(() {
                  _qaBannerExpanded = !_qaBannerExpanded;
                });
              },
                ...
                child: Padding(
                  ...
                  child: ConstrainedBox(
                    constraints: BoxConstraints(
                      maxWidth: _qaBannerExpanded ? 330 : 170,
                    ),
                    child: Text(
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.args.isBattle != true) {
      ...
      final currentUserId = _currentAuthUserId();
      final viewerId =
          currentUserId.isNotEmpty ? currentUserId : _resolvedJoinUserId();
    ...
    final currentUserName =
        (firebaseUser?.displayName ?? widget.args.hostName ?? '').trim();

    // Check if user is a participant (Host OR Challenger) using IDs, not roles
    final isParticipant = effectiveUserId == battle.competitor1Id ||
        effectiveUserId == battle.competitor2Id;
