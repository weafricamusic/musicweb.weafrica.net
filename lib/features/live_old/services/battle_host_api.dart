...
class BattleHostApi {
  ...
  /// Accept a challenge invite (host accepts battle request from artist/DJ)
  Future<HostedBattle> acceptChallenge({
    required String inviteId,
  }) async {
    final uri = _uriBuilder.build('/api/battle-invites/$inviteId/accept');

    final res = await _postJsonAuth(
      uri,
      body: <String, Object>{
        'invite_id': inviteId,
      },
    ...
    final body = res.body;
    final message = _messageFromBody(body).isEmpty 
        ? 'Could not accept challenge (${res.statusCode})' 
