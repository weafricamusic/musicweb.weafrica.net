...
class LiveSessionService {
  ...
  Future<AppResult<LiveSession>> joinSession(
    ...
    try {
      ...
      if (resolvedResponse == null || resolvedResponse.isEmpty) {
        if (explicitBattleId.isNotEmpty) {
          final battle = await _supabase
              .from('live_battles')
              .select('battle_id,channel_id,host_a_id,status')
              .eq('battle_id', explicitBattleId)
              .maybeSingle();

          final battleChannelId = (battle?['channel_id'] ?? '').toString().trim();
          if (battleChannelId.isNotEmpty) {
            resolvedResponse = <String, dynamic>{
              'id': battleChannelId,
              'channel_id': battleChannelId,
              'host_id': (battle?['host_a_id'] ?? '').toString(),
              'host_name': 'Artist',
              'title': 'Battle',
              'viewer_count': 0,
              'gift_count': 0,
      ...
      final channelId = (resolvedResponse['channel_id'] ?? resolvedResponse['id'] ?? '')
          .toString()
          .trim();
      if (channelId.isEmpty) {
        return const AppFailure(
      ...
      final hostId = (resolvedResponse['host_id'] ?? '').toString().trim();

      // 🔒 BROADCAST PERMISSION GATE
      if (asBroadcaster) {
        final role = 'artist'; // TODO connect Firebase/Supabase role
      ...
      final accessTier = (resolvedResponse['access_tier'] ?? '').toString();
      ...
      final isLive = resolvedResponse['is_live'] == true;
      final endedAt = resolvedResponse['ended_at'] != null
          ? DateTime.tryParse(resolvedResponse['ended_at'].toString())
          : null;
      ...
      final hostName = (resolvedResponse['host_name'] ?? '').toString().trim();
      final resolvedHostName = hostName.isNotEmpty ? hostName : 'Host';

      final liveIdRaw = (resolvedResponse['id'] ?? '').toString().trim();
      final liveId = liveIdRaw.isEmpty ? null : liveIdRaw;

      return AppSuccess(
        LiveSession(
          id: channelId,
          title: (resolvedResponse['title'] ?? '').toString(),
          hostName: resolvedHostName,
          opponentName: null,
          viewerCount: (resolvedResponse['viewer_count'] as int?) ?? 0,
          giftCount: (resolvedResponse['gift_count'] as int?) ?? 0,
          status: status,
          scheduledAt: resolvedResponse['scheduled_at'] != null
              ? DateTime.tryParse(resolvedResponse['scheduled_at'].toString())
              : null,
          channelId: channelId,
          token: token,
          userRole: shouldBroadcast ? UserRole.host : UserRole.audience,
          liveType: (resolvedResponse['live_type'] ?? '').toString().trim().isNotEmpty
              ? (resolvedResponse['live_type'] ?? '').toString().trim()
              : 'normal',
          liveId: liveId,
        ),
      );
