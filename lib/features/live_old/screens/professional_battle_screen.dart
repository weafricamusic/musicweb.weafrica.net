...
class _ProfessionalBattleScreenState extends State<ProfessionalBattleScreen>
  ...
  Future<void> _loadPerformerModePreference() async {
    ...
    try {
      ...
      final value = prefs.getBool(_performerModePrefKey()) ?? true;
      if (!mounted) return;
      setState(() {
  ...
  Widget _buildBattleStage(BuildContext context, BattleController battle) {
    ...
    final role = _resolveUserRole();
    final isPerformer = role == UserRole.host || role == UserRole.opponent;
    ...
    return Center(
      child: Container(
        ...
        child: Stack(
          children: [
            // Performers see full-screen self with PIP opponent
            if (isPerformer)
              _buildPerformerBattleView(battle, role)
            else
              // Audience sees 50/50 split
              Positioned.fill(
                child: BattleSplitView(
                  // Competitor1 on left, Competitor2 on right
                  competitor1Name: widget.competitor1Name,
                  competitor2Name: _effectiveCompetitor2Name,
                  competitor1Type: widget.competitor1Type,
                  competitor2Type: widget.competitor2Type,
                  isCompetitor1Leading: battle.competitor1Score > battle.competitor2Score,
                  isCompetitor2Leading: battle.competitor2Score > battle.competitor1Score,
                  rtcEngine: _streamController.engine,
                  channelId: widget.channelId,
                  remoteUids: _streamController.remoteVideoUids,
                  competitor1AgoraUid: _stableAgoraUid(widget.competitor1Id),
                  competitor2AgoraUid: _effectiveCompetitor2Id.isEmpty
                      ? null
                      : _stableAgoraUid(_effectiveCompetitor2Id),
                  localIsCompetitor1: widget.currentUserId.trim() == widget.competitor1Id.trim(),
                  localIsCompetitor2: widget.currentUserId.trim() == _effectiveCompetitor2Id,
                ),
              ),
            Positioned.fill(
              child: DecoratedBox(
  ...
  Widget _buildPerformerBattleView(BattleController battle, UserRole role) {
    final isHost = role == UserRole.host;
    final selfName = isHost ? widget.competitor1Name : _effectiveCompetitor2Name;
    final opponentName = isHost ? _effectiveCompetitor2Name : widget.competitor1Name;
    final selfType = isHost ? widget.competitor1Type : widget.competitor2Type;
    final opponentType = isHost ? widget.competitor2Type : widget.competitor1Type;
    final selfUid = isHost ? _stableAgoraUid(widget.competitor1Id) : _stableAgoraUid(_effectiveCompetitor2Id);
    final opponentUid = isHost ? (_effectiveCompetitor2Id.isEmpty ? null : _stableAgoraUid(_effectiveCompetitor2Id)) : _stableAgoraUid(widget.competitor1Id);
    final opponentAvatar = isHost ? _competitor2AvatarUrl : _competitor1AvatarUrl;
    final selfScore = isHost ? battle.competitor1Score : battle.competitor2Score;
    final opponentScore = isHost ? battle.competitor2Score : battle.competitor1Score;

    final engine = _streamController.engine;
    final remoteUids = _streamController.remoteVideoUids;
    final opponentConnected = opponentUid != null && remoteUids.contains(opponentUid);

    return Positioned.fill(
      child: Stack(
        children: [
          // Full-screen self video
          if (engine != null)
            AgoraVideoView(
          ...
          // PIP opponent video (top-right corner)
          if (opponentConnected && engine != null)
            Positioned(
              top: 12,
              right: 12,
              width: 100,
              ...
              child: Container(
                decoration: BoxDecoration(
                  ...
                  border: Border.all(
                    color: opponentScore > selfScore ? WeAfricaColors.gold : Colors.white.withValues(alpha: 0.3),
                    width: opponentScore > selfScore ? 2 : 1,
                  ),
                  boxShadow: [
                    BoxShadow(
            ...
            Positioned(
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    Icon(
                      opponentType.toLowerCase() == 'artist' ? Icons.mic : Icons.album,
