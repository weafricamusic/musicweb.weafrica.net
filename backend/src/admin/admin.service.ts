...
import { AgoraService } from '../stream/agora/agora.service';
...
export class AdminService {
  ...
  async makeAdmin(userId: string, role: 'viewer' | 'moderator' | 'admin' | 'super_admin', adminId: string) {
  ...
  async getReportedContent(status?: string, limit = 50) {
  ...
  async getViralAlerts(acknowledged?: boolean) {
  ...
  async acknowledgeViralAlert(alertId: string, adminId: string) {
    const { data, error } = await this.supabase.client
      .from('viral_alerts')
      .update({
  ...
  async getContentFlags(status?: string, limit = 50) {
  ...
  async createContentFlag(contentType: string, contentId: string, reason: string, severity = 1, adminId: string) {
    const { data, error } = await this.supabase.client
      .from('content_flags')
      .insert({
  ...
  async resolveContentFlag(flagId: string, action: 'dismiss' | 'remove', notes: string | undefined, adminId: string) {
  ...
  async getFinanceTransactions(limit = 100, offset = 0, type?: string) {
  ...
  async processWithdrawal(withdrawalId: string, action: 'approve' | 'reject' | 'mark_paid', notes: string | undefined, adminId: string) {
  ...
  async getAnalyticsOverview(days = 7, country?: string) {
    ...
    const safeCount = async (table: string, apply?: (query: any) => any): Promise<number | null> => {
      try {
    ...
    const safeList = async (table: string, select: string, apply?: (query: any) => any, limit = 5000): Promise<any[] | null> => {
      try {
    ...
    const safeRpc = async <T>(fn: string, args: Record<string, unknown>): Promise<T | null> => {
      try {
        const { data, error } = await (this.supabase.client as any).rpc(fn, args);
        if (error) return null;
    ...
    const daySeriesSum = (rows: any[] | null, dateCol: string, valueCol: string) => {
      if (!rows) return null;
    ...
    const daySeriesCount = (rows: any[] | null, dateCol: string) => {
      if (!rows) return null;
      const byDay = new Map<string, number>();
      for (const row of rows) {
        const day = isoDay(row?.[dateCol]);
    ...
    const sumNumber = (rows: any[] | null, key: string) => {
      if (!rows) return null;
      return rows.reduce((sum, row) => sum + (Number.isFinite(Number(row?.[key])) ? Number(row?.[key]) : 0), 0);
    };

    const sumByKey = (rows: any[] | null, keyCol: string, valueCol: string) => {
      if (!rows) return null;
    ...
    const transactions = await safeList(
      'transactions',
      'type,amount_mwk,coins,created_at,country_code',
      (query) => query.gte('created_at', rangeStart).in('type', revenueTypes).order('created_at', { ascending: false }),
    );
    if (transactions === null) warnings.push('transactions table not accessible');

    const withdrawals = await this.getWithdrawals('pending').catch(() => null);
    if (withdrawals === null) warnings.push('withdrawals not accessible');
    ...
    if (profileRows === null) warnings.push('profiles time series not available');
    ...
    if (openReports === null) warnings.push('content_reports table not accessible');
    ...
    if (activeStreams === null) warnings.push('live_sessions table not accessible');

    const recentStreams = await safeList(
      'live_sessions',
      'viewer_count,started_at,is_live,region',
      (query) => query.gte('started_at', rangeStart).order('started_at', { ascending: false }),
      2500,
    );
    if (recentStreams === null) warnings.push('live_sessions time series not available');
    ...
    const dau1d = await safeRpc<number>('analytics_distinct_users', {
      p_since: new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString(),
      p_event_name: 'app_open',
      p_country_code: country ?? null,
    });
    const mau30d = await safeRpc<number>('analytics_distinct_users', {
      p_since: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString(),
      p_event_name: 'app_open',
      p_country_code: country ?? null,
    });
    if (dau1d === null || mau30d === null) warnings.push('behavior telemetry not available');

    const joinTelemetry = await safeRpc<any>('analytics_stream_join_success_rate', {
      p_days: normalizedDays,
      p_country_code: country ?? null,
    });
    if (!joinTelemetry) warnings.push('stream join telemetry not available');
  ...
  async getStreams(status = 'live', region?: string, limit = 250) {
    let query = this.supabase.client
      .from('live_sessions')
      .select('*')
  ...
  async stopStream(streamId: string, reason: string, adminId: string) {
    ...
    let kickRuleId: number | null = null;
  ...
  async getSystemHealth() {
    ...
    const health = await Promise.all(
      services.map(async (service) => {
        ...
        if (error) throw error;
        return [service, data ?? { status: 'unknown' }] as const;
      }),
    );
  ...
  async recordHealthCheck(service: string, status: 'healthy' | 'degraded' | 'down', responseTimeMs?: number, error?: string) {
  ...
  async getHealthHistory(service: string, hours = 24) {
  ...
  private async removeContent(contentType: string, contentId: string, adminId: string, reason?: string) {
    if (contentType === 'song') {
  ...
  private async tryGetFinanceCoreSummary(startIso: string) {
    try {
      ...
      if (transactions.error || withdrawals.error) return null;
  ...
  private async tryGetFinanceCoreWithdrawals(status?: string) {
    try {
  ...
  private async enrichFinanceCoreWithdrawals(rows: any[]) {
    ...
    if (artistIds.length) {
      ...
      (data ?? []).forEach((row) => artistNames.set(String(row.id), String((row as any).stage_name ?? row.name ?? 'Artist')));
    }

    if (djIds.length) {
      const { data } = await this.supabase.client.from('djs').select('id,name,stage_name').in('id', djIds);
      (data ?? []).forEach((row) => djNames.set(String(row.id), String((row as any).stage_name ?? row.name ?? 'DJ')));
    }
    ...
    return rows.map((row) => ({
      ...
      display_name:
        String(row.beneficiary_type) === 'artist'
      ...
      admin_email: (row.admin_email ?? null) as string | null,
      note: (row.note ?? null) as string | null,
      source_table: 'withdrawals',
    }));
  }

  private async tryProcessFinanceCoreWithdrawal(
    withdrawalId: string,
  ...
  private async enrichStreams(rows: any[]) {
    ...
    return rows.map((row) => {
      ...
      return {
        ...
        streamer_name: String(
          hostProfile?.stage_name ?? hostProfile?.dj_name ?? hostProfile?.name ?? row?.host_name ?? row?.title ?? 'Creator',
        ),
        streamer_avatar_url: (hostProfile?.avatar_url ?? hostProfile?.photo_url ?? hostProfile?.profile_image_url ?? row?.thumbnail_url ?? null) as string | null,
        host_type: hostType,
        ...
        access_mode: (row?.access_mode ?? row?.access_tier ?? null) as string | null,
        raw: row,
      };
    });
  }
