-- Enforce same-role battle invites (artist↔artist, dj↔dj)
--
-- Requirement:
-- - Artists can invite only artists
-- - DJs can invite only DJs
-- - Consumers/admins cannot send/receive battle invites
--
-- This is enforced server-side in the battle_invite_create RPC.
...
begin
  ...
  -- Role enforcement: only same-role creator invites.
  select lower(trim(coalesce(p.role, ''))) into from_role
  from public.profiles p
  where p.id = from_uid_clean;

  if not found then
    raise exception 'from_profile_not_found';
  end if;

  select lower(trim(coalesce(p.role, ''))) into to_role
  from public.profiles p
  where p.id = to_uid_clean;

  if not found then
    raise exception 'to_profile_not_found';
  end if;

  if from_role not in ('artist','dj') then
    raise exception 'invalid_from_role';
  end if;
  if to_role not in ('artist','dj') then
    raise exception 'invalid_to_role';
  end if;
