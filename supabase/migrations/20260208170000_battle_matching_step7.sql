...
-- Upgrade safety: if the table existed already, ensure required columns exist.
alter table public.battle_invites add column if not exists battle_id text;
alter table public.battle_invites add column if not exists from_uid text;
alter table public.battle_invites add column if not exists to_uid text;
alter table public.battle_invites add column if not exists status text;
alter table public.battle_invites add column if not exists created_at timestamptz;
alter table public.battle_invites add column if not exists expires_at timestamptz;
alter table public.battle_invites add column if not exists responded_at timestamptz;
-- Best-effort defaults/backfills.
alter table public.battle_invites alter column created_at set default now();
update public.battle_invites set created_at = coalesce(created_at, now()) where created_at is null;
update public.battle_invites set status = coalesce(nullif(trim(status), ''), 'pending') where status is null or trim(status) = '';
...
begin
  if not exists (
    ...
    where schemaname = 'public'
      ...
      and policyname = 'deny_all_battle_invites'
  ) then
    create policy deny_all_battle_invites
...
-- Upgrade safety.
alter table public.battle_queue add column if not exists role text;
alter table public.battle_queue add column if not exists country text;
alter table public.battle_queue add column if not exists joined_at timestamptz;
alter table public.battle_queue alter column joined_at set default now();
update public.battle_queue set joined_at = coalesce(joined_at, now()) where joined_at is null;
...
begin
  if not exists (
    ...
    from pg_policies
    where schemaname = 'public'
      and tablename = 'battle_queue'
      and policyname = 'deny_all_battle_queue'
  ) then
    create policy deny_all_battle_queue
...
-- Upgrade safety.
alter table public.battle_match_events add column if not exists user_id text;
alter table public.battle_match_events add column if not exists battle_id text;
alter table public.battle_match_events add column if not exists created_at timestamptz;
alter table public.battle_match_events add column if not exists consumed_at timestamptz;
alter table public.battle_match_events alter column created_at set default now();
update public.battle_match_events set created_at = coalesce(created_at, now()) where created_at is null;
...
begin
  if not exists (
    ...
    from pg_policies
    where schemaname = 'public'
      and tablename = 'battle_match_events'
      and policyname = 'deny_all_battle_match_events'
  ) then
    create policy deny_all_battle_match_events
      on public.battle_match_events
      for all
      using (false)
      with check (false);
  end if;
